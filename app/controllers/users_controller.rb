class UsersController < ApplicationController
  ...
  def index
    if current_user.manager?
      ...
      if params[:q]
        @users = User.same_division.where("name LIKE ? OR email LIKE ? OR division_id LIKE ?", "%#{params[:q]}%", "%#{params[:q]}%", "%#{params[:q]}%" ).paginate(page: params[:page], per_page: Settings.users.page)
      else
        @users
      end
    else
